...
object GPUImageFilterTools {
    fun showDialog(
        ...
        val filters = FilterList().apply {
            addFilter("SourlScale(灵魂出窍)", FilterType.SOULSCALE)
    ...
    private fun createFilterForType(context: Context, type: FilterType): GPUImageFilter {
        return when (type) {
            FilterType.SOULSCALE -> SoulScaleFilter()
    ...
    private enum class FilterType {
        SOULSCALE,CONTRAST, GRAYSCALE, SHARPEN, SEPIA, SOBEL_EDGE_DETECTION, THRESHOLD_EDGE_DETECTION, THREE_X_THREE_CONVOLUTION, FILTER_GROUP, EMBOSS, POSTERIZE, GAMMA, BRIGHTNESS, INVERT, HUE, PIXELATION,
